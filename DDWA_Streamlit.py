...
#     Prediction code
    if st.button('Predict Price'):
        ...
        r = requests.post('http://127.0.0.1:5000/cars',json={'wheelbase':float(wheelbase_selection), 'carlength':float(carlenght_selection), 'carwidth':float(carwidth_selection), 'carheight':float(carheight_selection), 'curbweight':int(curbweight_selection),
           'enginesize':int(engine_selection), 'boreratio':float(boreratio_selection), 'stroke':float(stroke_selection),'compressionratio':float(compression_selection), 'horsepower':int(horsepower_selection),
           'peakrpm':int(peakrpm_selection), 'price':int(output)})
        resp = str(r)
        json_obj = resp.split('[')
        obj1 = json_obj[1].split(']')
        if obj1[0] == '200':
            st.write("New row inserted in the database")
        elif obj1[0] == '500':
